...
Limites connues du preset Quick Drive (pas de champ correspondant trouvé dans le schéma) : évolution du grip non mappée (toujours « Optimum »/sec), durée de session Practice non appliquée (sessions à durée libre par design Quick Drive), **qualification jamais désactivable** en mode Weekend (curseur CM `[5, 90]` min, aucun état « off » — voir §9.3).
