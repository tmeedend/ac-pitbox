...
**Les alias de pays sont une seconde table, et ne se confondent pas avec la première.** L'extraction **devine** un pays absent à partir d'un tag et ne joue que si le champ natif est vide ; les alias **normalisent** un pays déjà déclaré (« ce mod dit `U.S.A.`, c'est-à-dire United States ») et jouent toujours. Les fusionner laisserait un tag réécrire ce que l'auteur a pris la peine de déclarer. L'extraction s'édite dans l'écran Règles ; **les alias, dans l'onglet Pays de l'Atelier** (TAXO§8 : une table de correspondance exacte n'est pas une règle conditions → actions — c'est la même table dans le même fichier, rien n'a été converti). Ils vivent **hors des deux familles** : un circuit déclare un pays comme une voiture, et l'écrit tout aussi librement.
