...
**Le filtre Pays porte les drapeaux du jeu**, dans la liste de l'éditeur, sur les jetons posés et sur la puce de la barre — un pays se reconnaît à son drapeau avant de se lire. La table est celle d'Assetto Corsa (`nationalities.rs`, la même que la colonne Nationalité du plateau) et la correspondance se fait **par le nom, insensible à la casse** : les valeurs viennent des `ui_*.json` des mods, où l'auteur écrit ce qu'il veut.

**La table d'alias est mesurée, pas devinée** (`flags.ts`). Relevé sur les 575 `ui_*.json` d'une bibliothèque réelle : 17 orthographes de pays, dont cinq introuvables telles quelles dans la table du jeu, et à elles seules 82 des 395 mods qui portent un pays — `U.S.A.`, `USA` et `United States of America` pour les États-Unis, `Great Britain` pour le Royaume-Uni, plus une valeur cassée. **Les nations britanniques n'y sont pas** : AC donne son propre drapeau à l'Écosse, à l'Angleterre, au pays de Galles et à l'Irlande du Nord, et les renvoyer sur le Royaume-Uni remplacerait un drapeau juste par un autre drapeau juste.

**Une valeur cassée se lit, elle ne se répare pas.** Le `ui_track.json` de `le_lancone` porte `"country": "France\", \"Corsica"` — son auteur a voulu écrire deux entrées et en a produit une seule, guillemets compris. Le fichier du mod est en lecture seule (règle d'or n°1) : la valeur reste telle quelle en bibliothèque, mais le drapeau se lit sur ce qui précède le premier guillemet, qu'un nom de pays ne contient jamais. **Jamais à la virgule**, qui paraîtrait faire la même chose : la table du jeu en contient (`Tanzania, {United Republic of}`), et les couper les rendrait introuvables.

Un pays qui ne répond à aucune des trois passes garde son texte et n'a pas de drapeau — une correspondance approchée en poserait un faux, ce qui est pire. Dans la liste, une case vide tient alors la place, sans quoi les noms cesseraient de s'aligner.
