...
- **Réglage global** (préférence persistante, §11 — pas de question à chaque import) : **« Extraction des fichiers annexes »**, trois positions :
  ...
  - **Informations seulement** (défaut) — extrait uniquement les fichiers légers d'information : `.txt`, `.pdf`, `.md`, `.doc`/`.docx`, `.rtf`, `.nfo`, `.html`, `.url`, `.lnk`, ainsi que les **images** (`.jpg`/`.png`) — le tout à côté du dossier du mod uniquement (règle d'or ci-dessus). Une image posée à côté du mod est une capture de présentation ; la même image *dans* le mod est un asset AC (`logo.png`, `body_shadow.png`, `map.png`, aperçu de skin) et n'est jamais touchée.
