...
**Règle d'or — rien n'est jamais retiré de l'intérieur du dossier du mod.** Le « dossier du mod » est celui que l'auteur a conçu pour être posé dans `content/` (`rss_gtm_lanzo_v8/`, `ks_nordschleife/`), pas l'archive qui l'entoure. Tout ce qui est dedans, à quelque profondeur que ce soit, est du contenu du mod : copié en bibliothèque intégralement, jamais trié. **L'extraction des annexes ne porte que sur ce qui est à côté du dossier du mod** — racine de l'archive, dossiers frères. Une annexe repérée *dedans* (PDF de notice livré au milieu de la voiture) est **signalée sur la fiche, jamais déplacée** : dans le doute, le fichier reste où l'auteur l'a mis. Le critère est l'appartenance au mod, pas l'extension ni la profondeur — c'est en se fondant sur l'extension que `body_shadow.png`, `tyre_*_shadow.png` et `logo.png`, de vrais assets AC vivant à la racine du dossier voiture, ont été sortis de 23 mods. `scripts/audit-resources.ps1` audite et répare l'existant.

- **Extraction** : à l'import, les fichiers annexes (extensions non-jeu) situés **à côté du dossier du mod** sont rangés dans un sous-dossier **ressources** du mod **dans la bibliothèque** (jamais déployés dans `content/`). Le dossier Assetto reste propre, les annexes ne sont pas perdues.
...
- **Réglage global** (préférence persistante, §11 — pas de question à chaque import) : **« Extraction des fichiers annexes »**, trois positions :
  ...
  - **Informations seulement** (défaut) — extrait uniquement les fichiers légers d'information : `.txt`, `.pdf`, `.md`, `.doc`/`.docx`, `.rtf`, `.nfo`, `.html`, `.url`, `.lnk`. **Les images ne sont jamais des annexes**, à aucune profondeur et même à côté du mod : rien ne distingue une capture de présentation d'un asset AC (`logo.png`, `body_shadow.png`, `map.png`, aperçu de skin) — donc on ne tranche pas, on laisse. Une capture de présentation qui reste dans le mod ne coûte rien ; un `body_shadow.png` retiré casse le rendu.
