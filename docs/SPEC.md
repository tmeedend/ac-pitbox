...
**Filtre « Cacher le contenu de base »** : exclut le contenu Kunos de la liste, cases favoris/jamais essayé/contenu de base/mods non gérés regroupées et alignées verticalement dans la barre de filtres. Le filtre « contenu de base » teste `is_stock && !is_unmanaged`, jamais `is_stock` seul : le drapeau couvre tout ce qui vit dans `content/`, mods de l'utilisateur compris (§8). Un tri-état distinct isole ces derniers — c'est par lui qu'on retrouve, sur une install déjà moddée, ce que l'app ne gère pas.
