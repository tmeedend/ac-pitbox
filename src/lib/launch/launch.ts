...
import { PAUSE_SESSION, pauseGridThumbs } from "$lib/gridthumbs/gridThumbs.svelte";
...
// The opponent and its strength bounds live in `opponent.ts`, the recalibration
// rules of a saved setting in `setupRules.ts` — both loadable by Vitest, and
// re-exported here so that the screens keep one import.
export { AI_LEVEL_MIN, AI_LEVEL_MAX, clampAiLevel, AUTO_CELL, newOpponent, type Opponent } from "./opponent";
export { TRACK_GRIPS, GRIP_WEATHER, nearestGrip, START_MODES, assistLevelFrom } from "./setupRules";
