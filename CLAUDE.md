...
| `rules` / `categories` / `import` / `profiles` / `maintenance` | `workshop/Workshop.svelte` | **un écran, cinq onglets** — l'onglet EST la section, pas un état local |
...
local ; et `RulesEditor` reste le seul des cinq à gérer son propre
...
| **Taxonomies** | familles et onglet Catégories livrés ; restent les onglets Pays et Marques (fusions, logos canoniques) | `docs/SPEC-taxonomies.md`, `docs/CHANTIERS.md` |
